...
- Context, email, and board UI: Context tab in `components/context-view.tsx` with meeting drill-down in `components/meeting-context-detail.tsx` and bundle API `POST /api/context/meeting` (Soma, ctx gateway, Tower, Krisp OAuth MCP); email in `components/email-view.tsx` with Soma `/api/email/threads` and agent drafts via `POST /api/email/draft-reply`; Flightdeck board in `components/flightdeck-board-view.tsx` + `/api/flightdeck/board` (GitHub GraphQL via `lib/flightdeck-client.ts`, Tower MCP when configured).
