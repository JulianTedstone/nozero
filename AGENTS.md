...
- Context, email, and board are first-class top-level tabs alongside calendar. Meeting context detail is a zoom-in child of the Context tab (opened from calendar/HUD) with cross-links to email, Flightdeck, and calendar. Email and board follow the calendar three-column shell: email adds connected-account toggles, Unread·Tracking·All filters, search, Flightdeck stream tag cloud, and paginated inbox (~20 threads); board tab exposes GitHub Project #17.
